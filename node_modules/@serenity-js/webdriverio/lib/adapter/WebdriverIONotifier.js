...
class WebdriverIONotifier {
    ...
    notifyOf(event) {
        return (0, tiny_types_1.match)(event)
            ...
            .when(events_1.TestRunFinishes, WebdriverIONotifier.prototype.onTestRunFinishes.bind(this))
            .else(() => void 0);
    }
    onTestRunFinishes() {
    ...
    whenSuccessful(outcome, resultWhenSuccessful, resultWhenNotSuccessful) {
        return !outcome.isWorseThan(this.successThreshold) && (outcome instanceof model_1.ProblemIndication)
    ...
    invokeHooks(hookName, hookFunctions, args) {
        const hooks = !Array.isArray(hookFunctions)
            ? [hookFunctions]
            : hookFunctions;
        const asyncOperationId = model_1.CorrelationId.create();
        this.stage.announce(new events_1.AsyncOperationAttempted(new model_1.Description(`[WebdriverIONotifier#invokeHooks] Invoking ${hookName} hook`), asyncOperationId));
        return Promise.all(hooks.map((hook) => new Promise((resolve) => {
            ...
            then(results => {
            this.stage.announce(new events_1.AsyncOperationCompleted(new model_1.Description(`[WebdriverIONotifier#invokeHooks] Invoking ${hookName} hook completed`), asyncOperationId));
            return results;
        });
    }
}
 ...
 * @private
 */
WebdriverIONotifier.dummmyContext = {};
class EventLog {
    constructor() {
        this.events = new Map();
    }
